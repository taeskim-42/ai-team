...
# ── Persona → Agent mapping helpers ──
_persona_to_label() {
  case "$1" in
    dhh)              echo "DHH" ;;
    chris-lattner)    echo "Chris Lattner" ;;
    dan-abramov)      echo "Dan Abramov" ;;
    guillermo-rauch)  echo "Guillermo Rauch" ;;
    evan-you)         echo "Evan You" ;;
    ryan-dahl)        echo "Ryan Dahl" ;;
    rob-pike)         echo "Rob Pike" ;;
    guido-van-rossum) echo "Guido van Rossum" ;;
    graydon-hoare)    echo "Graydon Hoare" ;;
    james-gosling)    echo "James Gosling" ;;
    taylor-otwell)    echo "Taylor Otwell" ;;
    kelsey-hightower) echo "Kelsey Hightower" ;;
    linus-torvalds)   echo "Linus Torvalds" ;;
    doug-cutting)     echo "Doug Cutting" ;;
    kent-beck)        echo "Kent Beck" ;;
    *)                echo "$1" ;;
  esac
}
...
_persona_to_tech() {
  case "$1" in
    dhh)              echo "Rails · Ruby" ;;
    chris-lattner)    echo "Swift · iOS" ;;
    dan-abramov)      echo "React · Frontend" ;;
    guillermo-rauch)  echo "Next.js · Vercel" ;;
    evan-you)         echo "Vue · Frontend" ;;
    ryan-dahl)        echo "Node · Deno · Bun" ;;
    rob-pike)         echo "Go" ;;
    guido-van-rossum) echo "Python" ;;
    graydon-hoare)    echo "Rust" ;;
    james-gosling)    echo "Java" ;;
    taylor-otwell)    echo "PHP · Laravel" ;;
    kelsey-hightower) echo "Infra · DevOps" ;;
    linus-torvalds)   echo "Systems · C · Linux" ;;
    doug-cutting)     echo "Big Data · Java" ;;
    kent-beck)        echo "Testing · Quality Assurance" ;;
    *)                echo "" ;;
  esac
}
...
_persona_to_rules() {
  case "$1" in
    dhh)              echo "backend-rails" ;;
    chris-lattner)    echo "swift-ios" ;;
    dan-abramov)      echo "frontend-react" ;;
    guillermo-rauch)  echo "frontend-nextjs" ;;
    evan-you)         echo "frontend-vue" ;;
    rob-pike)         echo "backend-go" ;;
    guido-van-rossum) echo "backend-python" ;;
    graydon-hoare)    echo "backend-rust" ;;
    james-gosling)    echo "backend-java" ;;
    taylor-otwell)    echo "backend-php" ;;
    kelsey-hightower) echo "infra" ;;
    *)                echo "" ;;
  esac
}

_persona_to_agent_id() {
  local persona="$1" has_multiple_devs="$2"
  if [[ "$persona" == "kent-beck" ]]; then
    echo "qa"
  elif $has_multiple_devs; then
    # Determine frontend vs backend
    case "$persona" in
      dan-abramov|guillermo-rauch|evan-you|chris-lattner) echo "frontend-dev" ;;
      *) echo "backend-dev" ;;
    esac
  else
    echo "dev"
...
_save_config() {
  ...
  # Count non-QA dev personas to determine agent IDs
  local _dev_count=0
  for p in ${selected_personas[@]+"${selected_personas[@]}"}; do
    [[ "$p" != "kent-beck" ]] && _dev_count=$((_dev_count + 1))
  done
  local _has_multiple_devs=false
  [ "$_dev_count" -ge 2 ] && _has_multiple_devs=true

  {
    printf '#!/bin/bash\n'
    printf '# ══════════════════════════════════════════════════════════════\n'
    printf '# AI Team — Project Configuration\n'
    printf '# ══════════════════════════════════════════════════════════════\n'
    printf '# Generated by setup.sh\n'
    printf '# Edit values below, then run:  bash ai-team.sh\n'
    printf '# ══════════════════════════════════════════════════════════════\n\n'
    ...
    local i=1
    for p in ${selected_personas[@]+"${selected_personas[@]}"}; do
      local _aid _alabel _atech _arules _apersona
      _aid=$(_persona_to_agent_id "$p" "$_has_multiple_devs")
      _alabel=$(_persona_to_label "$p")
      _atech=$(_persona_to_tech "$p")
      _arules=$(_persona_to_rules "$p")

      # Use dev persona + rules if rules exist, otherwise use persona file directly
      if [ -n "$_arules" ]; then
        _apersona="dev"
      else
        _apersona="$p"
      ...
      [ -n "$_arules" ] && printf 'AGENT_%d_RULES="%s"\n' "$i" "$_arules"
    ...
    printf '# ── Feedback Pipeline (optional) ────────────────────────────\n'
    printf 'FEEDBACK_ENABLED=false\n'
  } > "$cfg_file"
...
if [[ "$MODE" == "config" ]]; then
  ...
  # Search existing configs for this project path
  _found_cfg=""
  for _cfg in "$PROJECTS_DIR"/*/team.config.sh; do
    [[ -f "$_cfg" ]] || continue
    # Match against all REPO_*_PATH entries
    while IFS='=' read -r _k _v; do
      _v="${_v%\"}" ; _v="${_v#\"}"
      [[ "$_v" == "$PROJECT_PATH" ]] && { _found_cfg="$_cfg"; break 2; }
    done < <(grep '^REPO_[0-9]*_PATH=' "$_cfg")
  done
...
_load_config_personas() {
  local cfg="$1"
  source "$cfg"
  # New config format uses REPO_1_PATH instead of PROJECT_PATH
  [[ -z "${PROJECT_PATH:-}" && -n "${REPO_1_PATH:-}" ]] && PROJECT_PATH="$REPO_1_PATH"
  [[ -z "${PROJECT_PATH:-}" ]] && { err "Config missing PROJECT_PATH"; exit 1; }
  # DESCRIPTION is optional in new config format
  DESCRIPTION="${DESCRIPTION:-${PROJECT_NAME:-$(basename "$PROJECT_PATH")}}"
  PROJECT_NAME="${PROJECT_NAME:-$(basename "$PROJECT_PATH")}"
  # Load language from config
  if [[ -n "${LANG_CODE:-}" ]]; then
    _lang_"$LANG_CODE" 2>/dev/null || _lang_en
  fi
  # Reconstruct personas from PERSONA_N or AGENT_N_PERSONA
  selected_personas=()
  _pi=1
  while true; do
    eval "_pval=\${PERSONA_${_pi}:-}"
    [[ -z "$_pval" ]] && eval "_pval=\${AGENT_${_pi}_PERSONA:-}"
    [[ -z "$_pval" ]] && break
...
# ══════════════════════════════════════════════════════════════
  ...
  for _cfg in "$PROJECTS_DIR"/*/team.config.sh; do
    [[ -f "$_cfg" ]] || continue
    grep -q '^PROJECT_PATH=\|^REPO_1_PATH=' "$_cfg" 2>/dev/null || continue
    _proj_configs+=("$_cfg")
    _pn=$(grep '^PROJECT_NAME=' "$_cfg" | head -1 | sed 's/^PROJECT_NAME=//;s/^"//;s/"$//')
    _pp=$(grep -m1 '^PROJECT_PATH=\|^REPO_1_PATH=' "$_cfg" | head -1 | sed 's/^[^=]*=//;s/^"//;s/"$//')
  ...
  if [[ ${#_proj_configs[@]} -gt 0 ]]; then
    ...
    if [[ "$_choice" =~ ^[0-9]+$ ]] && [ "$_choice" -ge 1 ] && [ "$_choice" -le "$_max" ]; then
      if [ "$_choice" -lt "$_max" ]; then
        # Existing project selected → launch directly
        _sel_idx=$((_choice - 1))
        exec bash "$SCRIPT_DIR/ai-team.sh" "${_proj_configs[$_sel_idx]}"
      fi
...
# Auto-launch ai-team.sh with the project config
_slug=$(echo "$PROJECT_NAME" | tr '[:upper:]' '[:lower:]' | \
  sed 's/[^a-z0-9]/-/g' | sed 's/--*/-/g' | sed 's/^-//;s/-$//')
_launch_cfg="$PROJECTS_DIR/$_slug/team.config.sh"
if [ -f "$_launch_cfg" ]; then
  exec bash "$SCRIPT_DIR/ai-team.sh" "$_launch_cfg"
else
