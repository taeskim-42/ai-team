...
# ══════════════════════════════════════════════════════════════
# Enforcement Hooks & CLAUDE.md Installation (per repo)
# ══════════════════════════════════════════════════════════════

HOOKS_SRC="$SCRIPT_DIR/hooks"

# Enable Agent Teams globally
...
CLAUSEEOF
    echo "  [repo $_ri] CLAUDE.md created"
  else
    echo "  [repo $_ri] CLAUDE.md already exists (skipped)"
  fi
done
...
PRINCIPLE
  ...
  # Append enforcement info
  cat >> "$WORKSPACE/pm-prompt.txt" << 'ENFORCE'

## Enforcement (automatic)
Hooks enforce: type check → tests → file size (<300 lines).
Protected files in .claude/hooks/ are read-only. Do NOT attempt to modify them.
ENFORCE
}
